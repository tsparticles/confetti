...
import { C as getLogger, _ as LimitMode, a as getStyleFromHsl, b as Circle, c as rangeColorToRgb, g as OutMode, h as ParticleOutType, i as getHslFromAnimation, l as Options, m as DrawLayer, o as getStyleFromRgb, p as loadOptions, r as alterHsl, s as rangeColorToHsl, u as loadParticlesOptions, v as HdrMode, w as EventType, x as Rectangle } from "./index-CEomVYk2.js";
	...
	#resolveBackgroundElement() {
		...
		if (typeof background.element === "string") {
			if (typeof document !== "undefined") {
				...
				if (typeof HTMLCanvasElement !== "undefined" && node instanceof HTMLCanvasElement || typeof HTMLVideoElement !== "undefined" && node instanceof HTMLVideoElement || typeof HTMLImageElement !== "undefined" && node instanceof HTMLImageElement) this.#backgroundElement = node;
		...
		} else if (typeof HTMLCanvasElement !== "undefined" && background.element instanceof HTMLCanvasElement || typeof OffscreenCanvas !== "undefined" && background.element instanceof OffscreenCanvas || typeof HTMLVideoElement !== "undefined" && background.element instanceof HTMLVideoElement || typeof HTMLImageElement !== "undefined" && background.element instanceof HTMLImageElement) this.#backgroundElement = background.element;
	}
...
var CanvasManager = class {
	...
	#initContext() {
		const container = this.#container, canSupportHdr = container.actualOptions.hdr.enable && safeMatchMedia("(color-gamut: p3)")?.matches && safeMatchMedia("(dynamic-range: high)")?.matches;
	...
	#removeHdrListeners() {
		if (!this.#hdrMediaListeners) return;
		for (const { handler, mql } of this.#hdrMediaListeners) mql.removeEventListener("change", handler);
		this.#hdrMediaListeners = void 0;
	}
	#repairStyle() {
		const element = this.domElement;
		if (!element) return;
	...
	#initPosition(position) {
		...
		switch (this.options.move.direction) {
			case MoveDirection.inside:
				...
				break;
			case MoveDirection.outside: this.outType = ParticleOutType.outside;
		}
...
var ParticlesManager = class {
	...
	addParticle(position, overrideOptions, group, initializer) {
		...
		if (limit > 0) switch (limitMode) {
			...
			case LimitMode.wait: if (currentCount >= limit) return;
		}
		try {
